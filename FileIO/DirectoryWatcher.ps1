# Taken and modified from  https://powershell.one/tricks/filesystem/filesystemwatcher

# find the path to the desktop folder:
$desktop = [Environment]::GetFolderPath('Desktop')

# specify the path to the folder you want to monitor:
$Path = "D:\Sage\X3ERPV12\Runtime\tmp"
...
# specify what folder you want to back up the file on the created event
$backupFolder = "D:\Sage\X3ERPV12\Runtime\tmp\bak"
...
try
{
  ...
  $action = {
    ...
    switch ($ChangeType)
    {
      'Changed'  { "CHANGE" }
      'Created'  { "CREATED"
        Copy-Item -Path $FullPath -Destination $backupFolder
      }
      'Deleted'  { "DELETED"
...
finally
{
  ...
  # Unregister all other events if you wantt to becareful, this shouldn't be run on production
  Get-EventSubscriber | Unregister-Event

  # List remaining subscribers
  Get-EventSubscriber
